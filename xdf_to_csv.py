"""
Converts XDF data to CSV format for more efficient analysis.
"""
...
for p in exp_participant_codes:
    ...
    streams, fileheader = pyxdf.load_xdf(d + '/vr_explorative/data/objective/raw/%s.xdf' % (p)) # Read streams
    ...
    # Export data
    eeg_participant.to_csv('data/objective/csv/eeg/%s_eeg.csv' % (participant), index = False, header = True, sep = ',', encoding = 'utf-8')
    emg_participant.to_csv('data/objective/csv/emg/%s_emg.csv' % (participant), index = False, header = True, sep = ',', encoding = 'utf-8')
    ecg_participant.to_csv('data/objective/csv/ecg/%s_ecg.csv' % (participant), index = False, header = True, sep = ',', encoding = 'utf-8')